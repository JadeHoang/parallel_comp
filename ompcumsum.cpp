...
//parallel analog of cumsum() susing OMP

//inpur vector x, number of sesired thread nth
...
RcppExport SEXP ompcumsum(SEXP x, SEXP nth)
{
  ...
  #pragma omp parallel
  {
    ...
    //do cumulative sums for my chunk
    double sum = 0;
    ...
    //find ajustment values
    //
    //first , make sure all the chunk cumsum
    //are ready
    #pragma omp barrier
    //only one thread need compute and 
    //accumulate the right hand end points
    #pragma omp single
    {
